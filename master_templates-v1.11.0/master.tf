...
resource "gzip_me" "cloudcontroller" {
  input = "${ var.cloudcontroller }"
}

resource "gzip_me" "cloudcontroller_key" {
  input = "${ var.cloudcontroller_key }"
}
...
resource "gzip_me" "cloud_controller_manager" {
  input = "${ data.template_file.cloud_controller_manager.rendered }"
}

data "template_file" "cloud_controller_manager" {
  template = "${ file( "${ path.module }/cloud-controller-manager" )}"
...
resource "gzip_me" "cloud_controller_manager_kubeconfig" {
  input = "${ data.template_file.cloud_controller_manager_kubeconfig.rendered }"
}

data "template_file" "cloud_controller_manager_kubeconfig" {
  template = "${ file( "${ path.module }/kubeconfig" )}"
  ...
  vars {
    ...
    name                = "service-account-context"
    user_authentication = "client-certificate: /etc/srv/kubernetes/pki/cloudcontroller.crt \n    client-key: /etc/srv/kubernetes/pki/cloudcontroller.key"
  }
}
...
data "template_file" "master" {
  ...
  vars {
    ...
    cloud_controller_manager           = "${ element(gzip_me.cloud_controller_manager.*.output, count.index) }"
    ...
    cloudcontroller                    = "${ gzip_me.cloudcontroller.output }"
    cloudcontroller_key                = "${ gzip_me.cloudcontroller_key.output }"
    ...
    cloud_controller_manager_kubeconfig = "${ gzip_me.cloud_controller_manager_kubeconfig.output }"
